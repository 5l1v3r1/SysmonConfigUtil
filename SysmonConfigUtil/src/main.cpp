...
char version_stuff[] = {0x0, 0x0, 0x0, 0x0 ,0x0, 0x0, 0x0, 0x0,
                        0x0, 0x0, 0x0, 0x0 ,0x0, 0x0, 0xf0, 0xa1};
...
{
  char v0; // bl
  LSTATUS ret_val_3; // eax
  LSTATUS ret_val_2; // eax
  LSTATUS ret_val_1; // eax
  void *ret_buf; // esi
  const OLECHAR *enabled_str; // esi
  const OLECHAR *enabled_disabled_str; // eax
  LSTATUS ret_val; // esi
  BYTE *rules_buf; // eax
  ...
  int v17; // edx
  DWORD v18; // [esp+14h] [ebp-234h]
  unsigned int rule_struct_arr[2] = { 0, 0 }; // [esp+18h] [ebp-230h]
  BYTE reg_svc_num[4]; // [esp+20h] [ebp-228h]
  BYTE Data[4]; // [esp+24h] [ebp-224h]
  HKEY phkResult; // [esp+28h] [ebp-220h]
  PRULES_STRUCT rule_struct_ptr = 0; // [esp+2Ch] [ebp-21Ch]
  void *rules_buf_cpy; // [esp+30h] [ebp-218h]
  DWORD regValue_size; // [esp+34h] [ebp-214h]
  WCHAR SubKey[0x108]; // [esp+38h] [ebp-210h]
  DWORD reg_val;

  *(DWORD *)reg_svc_num = 1;
  rules_buf_cpy = nullptr;
  v0 = 0;
  memset(&SubKey, 0, sizeof(SubKey));
  swprintf_s((WCHAR*)&SubKey, 0x104u, L"System\\CurrentControlSet\\Services\\%s\\Parameters", ServiceName);
  ret_val_3 = RegOpenKeyExW(HKEY_LOCAL_MACHINE, (WCHAR*)&SubKey, 0, 0x20019u, &phkResult);
  if ( !ret_val_3 )
  {
    ...
    *(DWORD *)Data = 0;
    regValue_size = 4;
    ret_val_2 = RegQueryValueExW(phkResult, L"Options", 0, &reg_val, Data, &regValue_size);
    if ( ret_val_2 )
    {
      if ( ret_val_2 != 2 )
    ...
    {
      wprintf(L"Failed to open %s configuration with incorrect type %d / %d\n", L"Options", rules_buf_cpy, 4);
    }
    regValue_size = 4;
    ret_val_1 = RegQueryValueExW(phkResult, L"HashingAlgorithm", 0, &reg_val, reg_svc_num, &regValue_size);
    if ( ret_val_1 )
    {
      if ( ret_val_1 != 2 )
        wprintf(L"Failed to open %s configuration with last error %d\n", L"HashingAlgorithm", ret_val_1);
    }
    else if ( reg_val != 4 )
    {
      wprintf(L"Failed to open %s configuration with incorrect type %d / %d\n", L"HashingAlgorithm", rules_buf_cpy, 4);
    }
    ret_buf = resolve_hashmap(*(unsigned int *)reg_svc_num);
    wprintf(L"%-34s%s\n", L" - HashingAlgorithms:", ret_buf);
    free(ret_buf);
    enabled_str = L"enabled";
    enabled_disabled_str = L"enabled";
    if ( !(Data[0] & 1) )
      enabled_disabled_str = L"disabled";
    wprintf(L"%-34s%s\n", L" - Network connection:", enabled_disabled_str);
    if ( !(Data[0] & 2) )
      enabled_str = L"disabled";
    wprintf(L"%-34s%s\n\n", L" - Image loading:", enabled_str);

    ret_val = RegQueryValueExW(phkResult, L"Rules", 0, &v18, 0, &regValue_size);
    if ( ret_val ){
    ...
    //Allocate memory
    rules_buf = (BYTE *)malloc(regValue_size);
    rules_buf_cpy = rules_buf;
    if ( !rules_buf ){
      wprintf(L"Failed to open rules configuration with last error %d\n", 14);
      if ( rules_buf_cpy )
		free((void *)rules_buf_cpy);
	  return;
    }

    ret_val = RegQueryValueExW(phkResult, L"Rules", 0, &v18, rules_buf, &regValue_size);
    if ( ret_val ){
...
//        goto LABEL_18;
      ...
      if ( get_struct_data_wrap( &rule_struct_ptr) )
      {
        ...
        if ( get_struct_data_wrap2( rule_struct_ptr, (unsigned int *)&rule_struct_arr) )
        {
          ...
          {
            wprintf(
              L"Rule configuration (version %.2f):\n",
            ...
            //buf_size = *((DWORD *)rule_struct_ptr + 2);
            if ( buf_size < 8 )
              goto LABEL_43;
			v11 = rule_struct_ptr->rules_buf;
           // v11 = *((DWORD *)rule_struct_ptr + 1);
            if ( !*(DWORD *)(v11 + 4) || v11 + 8 < v11 || v11 + 8 >= buf_size + v11 || v11 == -8 )
              goto LABEL_43;
            rule_ptr = (void **)(v11 + 8);
            do
            {
              ...
              if ( rule_func_ptr )
              {
                ...
                {
//                  if ( *(_DWORD *)i < (unsigned int)*rule_func_ptr
//                    && *(_DWORD *)(*((_DWORD *)rule_func_ptr + 7) + 4 * *(_DWORD *)i) )
//                  {
                    rule_modifier_ptr = get_rule_modifier(*(i + 4));
//                    wprintf(L"\t%-30s filter: %-12s value: '%s'\n", v17, rule_modifier_ptr, i + 16);
//                  }
                }
              }
...
//            rule_struct_ptr_cpy = rule_struct_ptr;
LABEL_43:
//            some_counter(rule_struct_ptr_cpy);
//LABEL_44:
            if ( rules_buf_cpy )
              free((void *)rules_buf_cpy);
            return;
...
unsigned char *iterate_rule(unsigned char *a1, PRULES_STRUCT *a2, unsigned char *a3)
{
  ...
  v3 = *a2;
  v4 = (unsigned char *)v3->rules_buf;
  if ( a1 < v4 )
    return 0;
  v5 = *(DWORD *)(v3 + 8);
  if ( a1 >= v5 + v4 || !*(DWORD *)(a1 + 12) )
    return 0;
  if ( a3 )
  {
    if ( a3 < v4 || a3 >= v5 + v4 )
...
PRULE_STRUCT get_rule_func( uint32_t rule_num) {
  ...
  while ( 1 )
  {
    ...
    //if ( *(DWORD *)(rule_obj + 40) )
    {
	    if ( rule_obj->RULE_ID_NUM == rule_num )
      //if ( *(void **)(rule_obj + 12) == some_ptr )
			break;
    }
    if ( (unsigned int)++iterator >= 9 )
      return 0;
  }
  return &rule_arr[iterator];
}
...
wchar_t *resolve_hashmap(unsigned int hash_reg_val)
{
  ...
  else
  {
    ...
    if ( new_buf )
    {
      ...
      if ( v4 )
      {
        while ( (!v11 || !wcscat_s(v10, SizeInWordsa, L",")) && !wcscat_s(v10, SizeInWordsa, Src[v11]) )
        {
          if ( ++v11 >= v4 )
            goto LABEL_14;
        }
